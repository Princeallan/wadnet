    ...
    public function map()
    {
        Route::namespace($this->namespace)->group(function (Router $router) {
                ...
                $router->middleware('public')->group(function (Router $router) use ($page) {
                    ...
                    foreach (locales() as $lang) {
                        if ($page->translate('status', $lang) && $uri = $page->uri($lang)) {
                            $router->get($uri, $options + ['uses' => 'PublicController@index'])->name($lang.'::index-contacts');
                            $router->get($uri.'/{slug}', $options + ['uses' => 'PublicController@show'])->name($lang.'::contact');
                        }
                    }
                });
            }
            ...
            $router->middleware('admin')->prefix('admin')->group(function (Router $router) {
                ...
                $router->get('contacts/{contact}/files', 'AdminController@files')->name('admin::edit-contact-files')->middleware('can:update-contact');
